...
بعد `git pull`، اتّبع هذه الأوامر بالضبط.

**⚠️ إذا ظهر خطأ 500 على nalp.fly.dev:** تأكد من تعيين `APP_KEY` (القسم C أدناه). لأي خطأ في `fly deploy` أو `fly logs` أرسله للتشخيص.
