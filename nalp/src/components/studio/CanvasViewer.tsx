...
/**
 * شارع 12.5م ثابت ملاصق للحافة الجنوبية — جنوب الأرض (مواءمة البوصلة: ج جنوب أسفل)
 * في المشهد: z=0 = جنوب، z=depth = شمال. الشارع عند z=0
 */
...
/** حصة الجار الجنوبي — أسفل الشارع الداخلي (z سالب) */
...
function LandPlanSketch({
  ...
  if (streetEnabled && netDepth > 0) {
    ...
    [
      ...
      if (w > 0.1 && zoneDepth > 0.1) {
        meshes.push(
          <mesh
            key={`zone-${key}`}
            position={[x + w / 2, 0.01, streetDepth + zoneDepth / 2]}
            rotation={[-Math.PI / 2, 0, 0]}
          >
            <planeGeometry args={[w, zoneDepth]} />
...
function Scene({ glbUrl, streetEnabled, landBounds, zonePercents, useReferenceSketch }: SceneProps) {
  ...
  return (
    <>
      ...
      <OrbitControls target={[len / 2, 0, depth / 2]} />
      <Grid
        args={[600, 600]}
        cellSize={10}
        cellThickness={0.4}
...
export function CanvasViewer({
  ...
  return (
    <Canvas camera={{ position: [260, 180, 32.5], fov: 45 }}>
      <Scene
        glbUrl={glbUrl}
        streetEnabled={streetEnabled}
        landBounds={landBounds}
