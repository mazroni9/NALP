...
export function ZoneALayoutPlan() {
  ...
  const roadCenterY = (gw / 2 + h - gateDepth / 2) / 2;
  ...
  const vbH = h + streetH * 2 + pad * 2 + 95;
    ...
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      >
        ...
        <g transform={`translate(${pad}, ${pad})`}>
          ...
          <circle cx={w - gateDepth - 40} cy={gw / 2} r={8} fill="#22c55e" stroke="#166534" strokeWidth={1.5} />
          <text x={w - gateDepth - 40} y={gw / 2} textAnchor="middle" dominantBaseline="middle" className="fill-white text-[12px] font-bold">1</text>
          <text x={w - gateDepth - 40} y={gw / 2 + 30} textAnchor="middle" className="fill-green-800 text-[11px]">دخول</text>
          ...
          <text x={w - gateDepth - 95} y={gw / 2} textAnchor="end" className="fill-green-800 text-[12px]">سيارات قادمة</text>
          ...
          {(() => {
            ...
            // منطقة تجميع للمخرج — ملاصقة لحد شارعنا 12.5 (فوق بوابات المخرج مباشرة)
            const collectZoneY = h - platformH - gateDepth - 2;
            ...
            return (
              <g>
                ...
                {Array.from({ length: rows }).map((_, rowIndex) => {
                  ...
                  if (rowY + spotH > platformY - 8) return null;
                  return (
                    <g key={rowIndex}>
                ...
                />
                <text
                  x={auctionX0 + auctionW / 2 + 25}
                  y={auctionY0 + 12 + 2 * rowStep - aisleW / 2}
                  textAnchor="start"
                  dominantBaseline="middle"
                  className="fill-green-700 text-[12px] font-semibold"
                >
                  تدفق شمال→جنوب
                </text>
                {/* ٣ توزيع: من العبارة إلى رقم ٣ ثم صفوف الشمال */}
                <path
                  d={`M ${w / 2} ${28} L ${auctionX0 + 35} ${auctionY0 + 30}`}
                  fill="none"
                  stroke="#10b981"
                  strokeWidth={2}
                  strokeDasharray="6 4"
                ...
                <text x={w / 2} y={18} textAnchor="middle" className="fill-emerald-700 text-[13px] font-semibold">توزيع على الصفوف العرضية</text>
                ...
                {/* مسار الخروج — مخرج ١: سهم مباشر بين كلمة المنصة ومخرج ١، رأس السهم بعيد قليلاً عن مخرج ١ */}
                <path
                  d={`M ${collectZoneX + (platformX - collectZoneX - 4) / 2 - 28} ${platformY + platformH / 2 - 2} L ${stripW + gw / 2} ${h - gateDepth - 18}`}
                  fill="none"
                  stroke="#f59e0b"
                  strokeWidth={2}
                  strokeDasharray="6 4"
                ...
                />
                <path
                  d={`M ${platformX + platformW + 4 + (collectZoneX + collectZoneW - platformX - platformW - 4) / 2} ${platformY + platformH / 2} L ${w - gw / 2 - 25} ${h - gateDepth - 15}`}
                  fill="none"
                  stroke="#f59e0b"
                  strokeWidth={1.5}
                  strokeDasharray="4 3"
                ...
                <text x={stripW + gw / 2} y={h - gateDepth - 28} textAnchor="middle" className="fill-amber-800 text-[13px] font-semibold">خروج ٦</text>
                {/* تسمية — في الممر بين صفوف السيارات (وليس فوق الصفوف) */}
                <text x={auctionX0 + auctionW / 2} y={auctionY0 + 12 + 4 * rowStep - aisleW / 2} textAnchor="middle" dominantBaseline="middle" className="fill-slate-600 text-[14px] font-semibold">صفوف عرضية (~150 سيارة) | تدفق شمال→جنوب | ساحة ≈4,100 م²</text>
                <text x={roadCenterX} y={roadCenterY} textAnchor="middle" dominantBaseline="middle" className="fill-slate-600 text-[13px] font-medium" transform={`rotate(-90, ${roadCenterX}, ${roadCenterY})`}>طريق داخلي 7م</text>
              </g>
            );
          })()}

          {/* مخرج ١ — يُرسم فوق العناصر ليكون ظاهراً */}
          <rect
            x={stripW}
            y={h - gateDepth}
            width={gw}
          ...
          <rect x={15} y={h + streetH * 2 - 2} width={w - 30} height={115} fill="#f8fafc" stroke="#94a3b8" strokeWidth={1} rx={4} />
          ...
          <text x={w / 2} y={h + streetH * 2 + 24} textAnchor="middle" className="fill-slate-600 text-[13px]">
            مدخل ١ ← طريق ٢ ← توزيع ٣ على الصفوف العرضية ← منصة ٤
          </text>
          <text x={w / 2} y={h + streetH * 2 + 40} textAnchor="middle" className="fill-slate-600 text-[13px]">
            تجميع يمين المنصة (مُباعة) → مخرج ٢ | تجميع يسار المنصة (لم تُبع — أصحابها غائبون) → مخرج ١
          </text>
          <text x={w / 2} y={h + streetH * 2 + 56} textAnchor="middle" className="fill-slate-600 text-[13px]">
            خروج ٦ ← مخرج ١ أو ٢ ← شارعنا
          </text>
          <text x={w / 2} y={h + streetH * 2 + 70} textAnchor="middle" className="fill-amber-700 text-[12px]">المُباعة تنتظر الخروج — غير المُباعة (أصحابها غائبون) تخرج عبر مخرج ١</text>
          <text x={w / 2} y={h + streetH * 2 + 88} textAnchor="middle" className="fill-slate-600 text-[11px]">المزاد يومي: السيارات تدخل وتُعرض وتُباع وتغادر في نفس اليوم. غير المُباعة أو التي تحتاج تأخيراً تغادر خلال ٢٤ ساعة</text>
          <text x={w / 2} y={h + streetH * 2 + 101} textAnchor="middle" className="fill-rose-700 text-[11px] font-medium">تأخير عن ٢٤ ساعة: رسوم ٢٠ ريال عن كل يوم أو جزء منه</text>
        </g>
      </svg>
    </div>
  );
